...
// No external hydro layer; "water_tailings" detected by attribute keywords only.
...
// ===== Load mines data (root-level on GitHub Pages) ===== //
fetch('./mines.geojson')
  .then(r => r.json())
  .then(geo => {
    L.geoJSON(geo, {
  ...
  .catch(err => {
    ...
    if (panel) panel.innerHTML = `<p style="color:#ff6b6b">Error: could not load <code>mines.geojson</code>. Make sure it is at the repo root.</p>`;
  });
...
function classifyImpact(lat, lng, props){
  ...
  for (const hub of INDIGENOUS_HUBS){
    if (haversineKm(lat, lng, hub.lat, hub.lng) <= 55) return "near_indigenous";
  }
  if (lat >= 51.5) return "near_indigenous";

  // 2) Population (30 km)
  for (const c of POP_CENTERS){
    if (haversineKm(lat, lng, c.lat, c.lng) <= 30) return "near_population";
  }

  // 3) Tailings / water by attributes only
...
function styleForImpact(impact){
  ...
  switch(impact){
    case "near_indigenous": return { ...base, color: "#ff6b6b", fillColor: "#ff6b6b" };
    case "near_population": return { ...base, color: "#4dabf7", fillColor: "#4dabf7" };
    case "water_tailings":  return { ...base, color: "#ffd166", fillColor: "#ffd166" };
    default:                return { ...base, color: "#ced4da", fillColor: "#ced4da" };
  }
}
...
if (filterEl){
  filterEl.addEventListener("change", (e) => {
    ...
    ALL_MARKERS.forEach(m => {
      if (want === "all" || m.featureData.impact === want) m.addTo(markerLayer);
    });
    updateStatsBox(want);
  });
}
